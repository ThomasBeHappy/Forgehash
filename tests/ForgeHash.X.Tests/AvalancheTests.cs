...
public sealed class AvalancheTests
{
    ...
    public void SingleBitFlip_ChangesAboutHalfTheOutputBits()
    {
        ...
        for (int sample = 0; sample < samples; sample++)
        {
            ...
            byte[] baseline = ForgeHashXApi.DeriveHash(password, salt, parameters);

            for (int bit = 0; bit < 2; bit++)
            {
                password[0] ^= (byte)(1 << bit);
                byte[] flipped = ForgeHashXApi.DeriveHash(password, salt, parameters);
                password[0] ^= (byte)(1 << bit);
