use forgeh::{derive_hash, derive_seed, encode, Params};
...
fn vectors_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../implementers/v1/vectors")
}
...
fn run_vector(file: &str) {
    ...
    assert_eq!(encode(1, &params, &salt, &hash), v.encoded, "encoded mismatch in {file}");
}
...
#[test]
fn vector2() {
    run_vector("vector2_password_incrementing_salt.json");
}

#[test]
fn vector3() {
    run_vector("vector3_utf8_two_lanes.json");
}

#[test]
fn vector4() {
    run_vector("vector4_null_bytes_four_lanes.json");
}
