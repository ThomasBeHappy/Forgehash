//! Experimental ForgeHash-B3 v1 — Rust reference.
//!
...
pub fn verify_password(password: &[u8], encoded: &str) -> bool {
    ...
    let params = Params {
        ...
        output_length: parsed.hash.len(),
        salt_length: parsed.salt.len(),
    };
    let actual = match derive_hash(password, &parsed.salt, &params) {
        Ok(h) => h,
        Err(_) => return false,
    };
...
/// C ABI for PHP / other FFI consumers.
pub mod ffi {
    use crate::{derive_hash, encode, params::Params, verify_password};
    use std::os::raw::{c_char, c_int, c_uchar};
    use std::slice;

    #[no_mangle]
    pub unsafe extern "C" fn forgeh_derive_seed(
        ...
        match crate::derive_seed(password, salt, &params) {
            Ok(seed) => {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn forgeh_derive_hash(
        password: *const c_uchar,
        password_len: usize,
        salt: *const c_uchar,
        salt_len: usize,
    ...
    #[no_mangle]
    pub unsafe extern "C" fn forgeh_verify_password(
        password: *const c_uchar,
        password_len: usize,
    ...
    #[no_mangle]
    pub unsafe extern "C" fn forgeh_encode(
        memory_kib: u32,
        iterations: u32,
        parallelism: u32,
        salt: *const c_uchar,
        ...
        if salt.is_null() || hash.is_null() || out.is_null() {
            return -1;
        }
        let salt = slice::from_raw_parts(salt, salt_len);
        let hash = slice::from_raw_parts(hash, hash_len);
        let params = Params {
            memory_kib: memory_kib as usize,
            iterations: iterations as usize,
            parallelism: parallelism as usize,
        ...
        let encoded = encode(1, &params, salt, hash);
